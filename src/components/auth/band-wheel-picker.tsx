...
export function BandWheelPicker({ value, onChange, className }: BandWheelPickerProps) {
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (isDragging) {
      ...
      // User tapped somewhere on the dial wheel
      const { cx, cy } = dragState.current;
      if (cx > 0 && cy > 0) {
        const tapAngle = Math.atan2(e.clientY - cy, e.clientX - cx) * (180 / Math.PI);
        const angleFromTop = (((tapAngle + 90) % 360) + 360) % 360;
        const dialAngle = (((angleFromTop - rotationRef.current) % 360) + 360) % 360;
        const tappedIdx = Math.round(dialAngle / stepAngle) % BAND_VALUES.length;
        const tappedBand = BAND_VALUES[tappedIdx];
        if (tappedBand !== valueRef.current) {
          valueRef.current = tappedBand;
          onChange(tappedBand);
          const targetAngle = -tappedIdx * stepAngle;
          const snapped = getClosestAngle(rotationRef.current, targetAngle);
          rotationRef.current = snapped;
          setRotation(snapped);
        }
  ...
  return (
    <div className={cn("relative flex flex-col items-center select-none py-2", className)}>
      ...
      <div className="relative w-[276px] h-[276px]">
        ...
        <div className="pointer-events-none absolute top-1 left-1/2 z-30 -translate-x-1/2 flex flex-col items-center">
          <div className="h-0 w-0 border-x-[7px] border-x-transparent border-t-[10px] border-t-brand-bright drop-shadow-md" />
        </div>

        {/* Outer Ring Ambient Glow - Perfectly concentric */}
        <div className="pointer-events-none absolute inset-0 m-auto w-[266px] h-[266px] rounded-full border border-brand-bright/25 bg-gradient-to-b from-brand-bright/10 via-transparent to-brand-bright/5 shadow-inner" />
        ...
        <div
          ...
          className={cn(
            "absolute inset-0 m-auto w-[240px] h-[240px] cursor-grab active:cursor-grabbing rounded-full border-2 border-border/90 bg-surface shadow-2xl outline-none focus-visible:ring-2 focus-visible:ring-brand-bright/50",
            isDragging
              ? "transition-none"
              ...
              <button
                ...
                className={cn(
                  ...
                  isDragging
                    ...
                    : "transition-transform duration-300 cubic-bezier(0.2, 0.8, 0.2, 1)",
                  isSelected
                    ? "bg-brand-bright text-white shadow-lg ring-2 ring-brand-bright/40 z-10 font-black"
                    : "text-muted-foreground hover:text-foreground hover:bg-muted/40"
                )}
                style={{
                  left: `${x}px`,
                  top: `${y}px`,
                  transform: `translate(-50%, -50%) rotate(${-rotation}deg) scale(${isSelected ? 1.2 : 1})`,
                  transformOrigin: "center center",
                }}
        ...
        <div className="pointer-events-none absolute inset-0 m-auto z-20 flex h-[98px] w-[98px] flex-col items-center justify-center rounded-full border border-brand-bright/30 bg-card/95 shadow-xl backdrop-blur-md">
      ...
      <div className="mt-3 flex items-center gap-3">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-surface text-muted-foreground transition-all hover:border-brand-bright/40 hover:text-foreground disabled:opacity-30 disabled:pointer-events-none"
